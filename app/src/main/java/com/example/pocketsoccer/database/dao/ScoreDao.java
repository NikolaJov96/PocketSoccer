package com.example.pocketsoccer.database.dao;
...
public interface ScoreDao {
    ...
    @Query("SELECT first_player_name, second_player_name, SUM(w1) AS first_player_wins, SUM(w2) AS second_player_wins FROM" +
            ...
            "   CASE WHEN first_player_score > second_player_score THEN 1 ELSE (" +
            "     CASE WHEN second_player_score > first_player_score THEN -1 ELSE 0 END " +
            "   ) END AS d" +
            "   FROM score)" +
            " GROUP BY first_player_name, second_player_name" +
            " ORDER BY ABS(SUM(d)) DESC")
